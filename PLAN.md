...
- [ ] Inventariar o que já foi alterado na pasta `scripts/` e confirmar se há cópias antigas do pipeline que possam servir de base.
- [ ] Mapear quais partes do Kiss3DGen precisamos replicar (preprocess, caption, multiview, bundle, reconstrução).
- [ ] Registrar dependências externas obrigatórias (Torch+CUDA, nvdiffrast, ninja, imagem base para teste) e onde serão instaladas.
...
- [ ] Executar/validar `download_models.py` (ou script equivalente) garantindo que todos os pesos exigidos pelo pipeline estejam em `models/`.
- [ ] Verificar integridade de cada submódulo necessário (por ex., `models/lrm`, `models/ISOMER`, `init_3d_Bundle`), registrando hashes ou datas.
- [ ] Preparar diretório de entrada (`data/inputs`) com pelo menos uma imagem de teste alinhada ao artigo.

#### 4. Adaptação do pipeline (sem importar código do Kiss3DGen)
- [ ] Reestruturar `scripts/run_kiss3dgen_image_to_3d.py` para manter apenas utilidades próprias (sem `from pipeline...`).
